...
public:
    ...
    int initializedMode; // 重定位模式，1为手动，2为自动
    float globalRelocalizeInitialFitnessScore;
    float globalRelocalizeFitnessScore;
    ...
    ParamServer()
    {
        ...
        nh.param<int>(PROJECT_NAME + "/initializedMode", initializedMode, 1);
        nh.param<float>(PROJECT_NAME + "/globalRelocalizeInitialFitnessScore", globalRelocalizeInitialFitnessScore, 0.25);
        nh.param<float>(PROJECT_NAME + "/globalRelocalizeFitnessScore", globalRelocalizeFitnessScore, 0.3);
    ...
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        //  6 轴
        Eigen::Quaterniond q_final = Q_quat_lidar;
